...
class OfficialVisitSearchIntegrationTest : IntegrationTestBase() {
  ...
  @Nested
  inner class ScheduledVisitSearch {
    @Test
  ...
  @Nested
  inner class CancelledVisitSearch {
    @Test
    fun `should find official cancelled visit by dates on one page`() {
      prisonerSearchApi().stubSearchPrisonersByPrisonerNumbers(
        listOf(MOORLAND_PRISONER.number),
        listOf(
          prisonerSearchPrisoner(
      ...
      testAPIClient.createOfficialVisit(nextMondayAt9, MOORLAND_PRISON_USER).also {
        testAPIClient.cancel(
          ...
          request = OfficialVisitCancellationRequest(
            ...
            cancellationNotes = "the visitor cancelled",
          ),
      ...
      val onePageOnly = webTestClient.search(searchRequest, MOORLAND_PRISON_USER, 0, 1)
      ...
      with(onePageOnly.content.single()) {
        visitSlotId isEqualTo 1L
  ...
  @Nested
  @Transactional
  inner class CompletedVisitSearch {

    @Test
    fun `should find official completed visit by dates on one page`() {
      prisonerSearchApi().stubSearchPrisonersByPrisonerNumbers(
        listOf(MOORLAND_PRISONER.number),
        listOf(
          prisonerSearchPrisoner(
      ...
      testAPIClient.createOfficialVisit(nextMondayAt9, MOORLAND_PRISON_USER).also { cv ->
        val scheduledVisit = officialVisitRepository.findById(cv.officialVisitId).orElseThrow()

        testAPIClient.complete(
          officialVisitId = scheduledVisit.officialVisitId,
          request = OfficialVisitCompletionRequest(
            completionReason = VisitCompletionType.NORMAL,
            completionNotes = "the visit completed normally",
            prisonerAttendance = AttendanceType.ATTENDED,
            prisonerSearchType = SearchLevelType.PAT,
      ...
      with(onePageOnly.content.single()) {
        ...
        visitStatusDescription isEqualTo "Completed"
        completionCode isEqualTo VisitCompletionType.NORMAL
        completionDescription isEqualTo "Normal completion"
        completionNotes isEqualTo "the visit completed normally"
        visitDate isEqualTo nextMondayAt9.visitDate
        startTime isEqualTo nextMondayAt9.startTime
        endTime isEqualTo nextMondayAt9.endTime
        dpsLocationId isEqualTo nextMondayAt9.dpsLocationId
