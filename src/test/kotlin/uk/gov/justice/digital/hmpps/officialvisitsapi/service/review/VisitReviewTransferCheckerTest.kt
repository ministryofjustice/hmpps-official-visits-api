...
class VisitReviewTransferCheckerTest {
  private val visitReviewRepository: VisitReviewRepository = mock()
  private val now = LocalDateTime.now()
  private val timeSource = TimeSource { now }
  private val transferChecker = VisitReviewTransferChecker(visitReviewRepository, timeSource)
  ...
  @Test
  fun `should be no new transfer details when pre-existing transfer visit review details`() {
    visitReviewDetail.stub { on { issueType } doReturn IssueType.PRISONER_TRANSFERRED }
    ...
    transferChecker.check(scheduledVisit)

    verify(visitReviewRepository, never()).saveAndFlush(any())
  }

  @Test
  fun `should be new transfer details when no pre-existing visit review details`() {
  ...
  @Test
  fun `should be new transfer details when no pre-existing visit review`() {
